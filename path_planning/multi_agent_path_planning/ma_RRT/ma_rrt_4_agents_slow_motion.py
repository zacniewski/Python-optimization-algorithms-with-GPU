...
VISUALIZE_EVERY = 10  # Update visualization every N iterations
PAUSE_TIME = 0.05  # Pause between frames (in seconds)
...
def init_visualization():
    ...
    plt.title("MA-RRT for 4 Agents - Path Construction")
...
def update_visualization(trees, iteration):
    colors = ['r', 'g', 'b', 'm']
    plt.clf()

    # Re-draw the base elements
    plt.xlim(0, MAP_SIZE[0])
    plt.ylim(0, MAP_SIZE[1])
    plt.title(f"MA-RRT for 4 Agents - Iteration {iteration}")
...
# MA-RRT Algorithm with visualization
def ma_rrt_with_visualization(start_positions, goal_positions):
...
# Run MA-RRT with visualization
trees = ma_rrt_with_visualization(START_POSITIONS, GOAL_POSITIONS)
